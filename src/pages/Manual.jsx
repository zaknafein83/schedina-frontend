...
const mdComponents = {
  h1: ({ children }) => (
    <h1 className="text-3xl font-black text-gds-white mt-2 mb-4">{children}</h1>
  ),
  h2: ({ children }) => (
    <h2 className="text-2xl font-bold text-gds-white mt-8 mb-3 pb-2 border-b border-gds-gray-light">
      {children}
    </h2>
  ),
  h3: ({ children }) => (
    <h3 className="text-lg font-bold text-gds-white mt-6 mb-2">{children}</h3>
  ),
  p: ({ children }) => (
    <p className="text-gds-white leading-relaxed mb-4">{children}</p>
  ),
  ul: ({ children }) => (
    <ul className="list-disc pl-6 mb-4 space-y-1 text-gds-white">{children}</ul>
  ),
  ol: ({ children }) => (
    <ol className="list-decimal pl-6 mb-4 space-y-1 text-gds-white">{children}</ol>
  ),
  li: ({ children }) => <li className="leading-relaxed">{children}</li>,
  strong: ({ children }) => (
    <strong className="font-semibold text-gds-white">{children}</strong>
  ),
  code: ({ children, className }) => {
    const isBlock = className?.includes('language-')
    if (isBlock) {
      return (
        <pre className="bg-gds-dark text-gds-gray rounded-lg p-4 overflow-x-auto text-sm mb-4">
          <code>{children}</code>
        </pre>
      )
    }
  ...
  th: ({ children }) => (
    <th className="bg-gds-gray-light text-left px-3 py-2 font-semibold border border-gds-border">
      {children}
    </th>
  ),
  td: ({ children }) => (
    <td className="px-3 py-2 border border-gds-border align-top">{children}</td>
  ),
}
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-3xl font-black text-gds-white">Guida</h1>
      </div>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 text-sm font-semibold border-b-2 transition-colors whitespace-nowrap ${
                active
                  ...
                  : 'border-transparent text-gds-gray hover:text-gds-white'
              }`}
            >
              <Icon size={16} />
              {label}
      ...
      <article className="bg-gds-surface rounded-xl shadow-sm border border-gds-gray-light p-6 sm:p-8">
